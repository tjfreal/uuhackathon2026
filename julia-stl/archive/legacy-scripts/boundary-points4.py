...
def process_ply_for_perimeter(ply_path):
    ...
    print("Identifying perimeter points using a 3x6 pixel window...")
    perimeter_points = []
    for point in points:
        x, y = point
        neighbors = [
        ...
        if not all(neighbors):  # Keep points missing at least one neighbor in the 3x6 window
    ...
    plt.scatter(perimeter_points[:, 0], perimeter_points[:, 1], s=1, color="red", label="Perimeter")
    plt.gca().invert_yaxis()
    plt.title("Perimeter Points of the Shape")
    plt.xlabel("X-axis")
    plt.ylabel("Y-axis")
    plt.legend()
    plt.show()
...
perimeter_points = process_ply_for_perimeter("julia_set_boundary.ply")
